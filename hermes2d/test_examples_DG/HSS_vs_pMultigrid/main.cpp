...
static std::string SolvedExampleString[5] = { "1D", "CircularConvection", "MovingPeak",
                                              "AdvectedCube", "SolidBodyRotation" };
...
//     V_cycles_per_time_step.push_back(2);
  ...
  // Exact solver solution
  {
    ...
    ss_global << "Grid=" << initialRefinementsCount << "|Diff=" << diffusivity << "|CFL=" 
              << CFL << "|Exact|Time=" << cpu_time.last();
  ...
  if (algorithm == Multiscale || algorithm == Both)
  {
    ...
    {
      std::stringstream ss;
      ss << "HSS(" << iter_per_time_step_HSS[si] << ")_" << solvedExampleString << "_" 
         << initialRefinementsCount << "_" << diffusivity << "_CFL=" << CFL << ".h2d";
      logger_HSS.set_logFile_name(ss.str());
      std::stringstream ssd;
      ssd << "HSS_detail(" << iter_per_time_step_HSS[si] << ")_" << solvedExampleString 
          << "_" << initialRefinementsCount << "_" << diffusivity << ".h2d";
      ...
      ss_global << "Grid=" << initialRefinementsCount << "|Diff=" << diffusivity << "|CFL=" << CFL 
                << "|HSS|Time=" << cpu_time.last() << "|" << outString;
      logger_global.info(ss_global.str().c_str());
    }
  }
  ...
  if(algorithm == pMultigrid || algorithm == Both)
  {
    ...
    for (int si = 0; si < smoothing_steps_per_V_cycle.size(); si++)
    {
     for (int siv = 0; siv < V_cycles_per_time_step.size(); siv++)
      {
        std::stringstream ss;
        ss << "MG(" << V_cycles_per_time_step[siv] << "-" << smoothing_steps_per_V_cycle[si] 
           << ")_" << solvedExampleString << "_" << initialRefinementsCount << "_" << diffusivity 
           << "_CFL=" << CFL << ".h2d";
        ...
        std::string outString = 
          p_multigrid(mesh, solvedExample, polynomialDegree, initialRefinementsCount, 
                      ...
                      smoothing_steps_per_V_cycle[si], CFL, V_cycles_per_time_step[siv]);
        // Stop measuring time.
        cpu_time.tick();
